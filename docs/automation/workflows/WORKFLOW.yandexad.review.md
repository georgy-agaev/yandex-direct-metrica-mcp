...
3. Inspect the workspace diff and relevant artifacts.
4. Inspect `SYMPHONY_STAGE_HANDOFF.md` when the current stage is expected to feed a later stage.
5. Re-run only the validation appropriate for the current stage:
...
6. Do not reject the current stage for missing later-stage validation.
6a. Do not reject a feature issue for missing repo-wide docs, changelog, release notes, PR copy, or downstream handoff documents unless the issue body explicitly requires them in `Feature Validation`.
7. Every transition out of review must be written into `SYMPHONY_HANDOFF.json`. Without a valid review handoff, do not move the issue to `Todo`, `Backlog`, or `Done`.
8. Before approving and creating a follow-up:
...
9. If you find issues:
   - write/update `SYMPHONY_HANDOFF.json` with:
     - `stage.role = review`
     - `transition.to_state = Todo` for fixable defects or `Backlog` for external blockers
     - `transition.status = needs_changes` or `blocked`
     - incremented `cycle.iteration`
