...
def render_file(template_path: Path, output_path: Path, symphony_root: Path) -> None:
    rendered = template_path.read_text(encoding="utf-8").replace(PLACEHOLDER, str(symphony_root))
...
def main() -> int:
    ...
    for name in TEMPLATE_FILES:
        render_file(template_root / name, output_root / name, symphony_root)
